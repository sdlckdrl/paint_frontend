...
<div>
      ...
      >
        ...
        <template v-slot:extension>
          ...
          >
            <v-tab v-for="(item, i) in menu" :key="i" :href="'#'+item.type" >{{item.title}}</v-tab>
          </v-tabs>
        </template>
          ...
          >
            ...
            <v-row>
              <v-col cols="5">
                <v-text-field
                  ...
                  v-model="num_x"
                  suffix="m"
                  type="number" @change="squareHandler"
                  onfocus="this.select()"
                ></v-text-field>
              </v-col>
              <v-col cols="1">
            ...
            <v-row>
              <v-col cols="12">
                ...
                >
                  면적은
                  <input style="width:50px" type="number" v-model="num_m2" @change="num_f=num_m2 / 3.3058;literHandler();" onfocus="this.select()"/>
                  ㎡ (
                    <input style="width:50px" type="number" v-model="num_f" @change="num_m2=num_f * 3.3058;literHandler();" onfocus="this.select()"/>평
                  )이며, 필요한 페인트 양은 대략 
                  <input style="width:50px" type="number" v-model="num_l" disabled/>
                  L 입니다.
                </v-alert>
              </v-col>
            </v-row>
...
export default {
  ...
  methods : {
    squareHandler : function(){
      this.num_m2 = this.num_x * this.num_y
      this.num_f = this.num_m2 / 3.3058
      this.literHandler()
    },    
    literHandler : function(){
      this.num_l = this.num_m2 / 5
    }
  },
  created(){
