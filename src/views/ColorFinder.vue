...
<div>
      ...
      >
        ...
        <v-btn icon @click="cropImage()" v-show="!cropped">
      ...
      <v-main >
          ...
          <v-layout wrap>
            <v-flex xs12>
              ...
              <v-card id="crop_img" v-show="cropped" class="ma-0 pa-0" width="100%" >
              </v-card>
...
export default {
  ...
  components: { VueCropper, InfiniteLoading, MenuComponent},
  data () {
    return {
      rgb_color:'',
      totalPage : 0,
  ...
  methods : {
    onEnter: function(){
      this.$router.push("/color_finder/"+this.rgb_color);
    },
    uploadFile : function(){
      let _this = this;
    ...
    cropImage : function(){    
      let _this = this;
      jQuery("#crop_img").empty()
      let canvas = this.$refs.cropper.getCroppedCanvas({
        width: jQuery('#crop_img').innerWidth(),
        height : jQuery('#crop_img').innerWidth()/2,
        imageSmoothingQuality:'high'
      })
      let context = canvas.getContext("2d");
      //this.crop_height = jQuery('#crop_img').innerWidth()/2
      jQuery('#crop_img').append(canvas)
      this.cropped=true;

      let _img;
      canvas.ontouchstart = function(e){
        e.preventDefault();
        var x = e.touches[0].pageX - jQuery('#crop_img').offset().left;
        var y = e.touches[0].pageY - jQuery('#crop_img').offset().top;
        _img = jQuery("<div style='' class='tmp_img'><div>")
        _img.css({
          "top":y-50, 
          "left":x-50,
          "position":"absolute",
      ...
      canvas.ontouchmove = function(e){
        ...
        _this.current_rgb_r = pixels[0];
        _this.current_rgb_g = pixels[1];
        _this.current_rgb_b = pixels[2];
        _img.css({
          "background-color": "rgb("+_this.current_rgb_r+","+_this.current_rgb_g+","+_this.current_rgb_b+")",
          "top":y-50, "left":x-50
          })
      ...
      canvas.ontouchend = function(e){
        e.preventDefault();
        var x = e.changedTouches[0].pageX - jQuery('#crop_img').offset().left;
        var y = e.changedTouches[0].pageY - jQuery('#crop_img').offset().top;
        var canvasColor = context.getImageData(x, y, 1,1); // rgba e [0,255]
        var pixels = canvasColor.data;
        _this.current_rgb_r = pixels[0];
        _this.current_rgb_g = pixels[1];
        ...
        _this.select_color = "background-color:rgb("+_this.current_rgb_r+","+_this.current_rgb_g+","+_this.current_rgb_b+")"
        _this.getLikeRgb();
        jQuery("#crop_img").find(".tmp_img").remove()
        var container = jQuery("#color_list")[0];
        container.scrollTop  = 0        
      }

      jQuery('#crop_img').height(jQuery(window).width()/2)
      jQuery("#color_list").height(jQuery(window).innerHeight() - (jQuery("#crop_img").innerHeight() + jQuery("footer").innerHeight() + jQuery("header").innerHeight() + jQuery("#combobox").innerHeight() + 100))
    },
    getLikeRgb : function(){
      this.sortList()
      this.updateCompany()
  ...
  created(){
    this.$http.get('/api/color_list/all').then((response) => {
      this.dataobj = response.data.filter((o)=>{return o.mt=="01"});
    })
  },
  filters:{
